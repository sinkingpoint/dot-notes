...
pub fn get_static_routes(
) -> impl Filter<Extract = impl warp::Reply, Error = warp::Rejection> + Clone {
    let get_note_path = warp::path!("note" / String).and(warp::get()).map(|_| {
    ...
    let get_main_page_path = warp::path::end().and(warp::get()).map(|| {
    ...
    let get_config_page_path = warp::path!("config").and(warp::get()).map(|| {
        let resp_body: String = generate_html("config_page-bundle.js").into();
        let mut resp = Response::new(resp_body);
        resp.headers_mut().insert(
            header::CONTENT_TYPE,
            header::HeaderValue::from_static("text/html"),
    ...
    return get_note_path
        .or(get_main_page_path)
        .or(get_config_page_path)
        .or(get_routes());
}
